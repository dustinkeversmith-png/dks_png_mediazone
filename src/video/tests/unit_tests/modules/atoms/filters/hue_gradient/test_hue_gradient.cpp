...
#include "filters/hue_gradient/hue_gradient.hpp"

#include <sstream>

// Atom demo: image in → CIE Lab ΔE hue/color gradient out.
class HueGradientAtom {
public:
    std::vector<LoadedSample> samples;
    AtomDemoReport report{"hue_gradient"};
    ...
    void run(const std::string& art_dir) {
        print_banner("run hue gradient → ΔE mag / gx / gy");
        ScopedTimer timer(&report.elapsed_ms);
        values_tsv << "file\tlabel\tmax_dE\tmean_dE\n";
        for (const auto& sample : samples) {
            contour::HueGradient hg;
            hg.compute(to_contour(sample.image));
            float mx = 0, sum = 0;
            for (float v : hg.mag.data) {
                mx = std::max(mx, v);
                sum += v;
            }
            const float mean = hg.mag.data.empty() ? 0.0f : sum / static_cast<float>(hg.mag.data.size());
            std::cout << "  " << sample.row.file << "  max_dE=" << mx << "\n";
    ...
    void write(const std::string& dir) {
        vision::write_text_file(vision::join_path(dir, "hue_gradient.tsv"), values_tsv.str());
        written.insert(written.begin(), "hue_gradient.tsv");
...
int main(int argc, char** argv) {
    constexpr const char* kDataset = "unit_hue_gradient";
    return run_atom_main(argc, argv, kDataset, [&](const AtomCli& cli) -> int {
        HueGradientAtom atom;
