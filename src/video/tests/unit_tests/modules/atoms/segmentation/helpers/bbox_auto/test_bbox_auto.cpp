...
// Atom: raw → threshold/open → CCL largest CC → AABB.
// Helpers: uses segmentation/helpers/ccl under the hood via BBoxAuto::from_binary_via_ccl.
class BBoxAutoAtom {
public:
    ...
    void run(const std::string& art_dir) {
        print_banner("run BBoxAuto → CCL largest-CC → AABB");
        ...
        for (size_t si = 0; si < samples.size(); ++si) {
            ...
            // Recipe: background removal / threshold → morph open → CCL → largest CC.
            auto raw_mask = mission_mask_image(ps, sample.image);
            if (raw_mask.empty()) {
                raw_mask = binarize_mask(luma);
            } else {
                raw_mask = binarize_mask(raw_mask);
            }
            const auto largest = contour::BBoxAuto::largest_component_mask(raw_mask, 127, 1);
            const contour::Rect box = contour::BBoxAuto::from_mask(largest);
            const auto crop = contour::BBoxAuto::crop(largest, box, 0.12f);
            const auto restored = contour::BBoxAuto::uncrop(crop.image, crop);
            const contour::Rect gt_box = contour::BBoxAuto::from_mask(largest);
            ...
            vision::save_pgm(vision::join_path(art_dir, stem + "_processed_base.pgm"), raw_mask);
            vision::save_pgm(vision::join_path(art_dir, stem + "_largest_cc.pgm"), largest);
            ...
            written.push_back(stem + "_largest_cc.pgm");
        ...
        report.notes.push_back(
            "threshold → morph-open → CCL largest CC → AABB; helpers: ccl + bbox_auto");
    }
