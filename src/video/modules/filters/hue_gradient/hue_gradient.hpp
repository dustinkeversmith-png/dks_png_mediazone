...
public:
    ...
    // Chrominance-only gradients (∇a*, ∇b*) — ignores lightness so isoluminant
    // color edges still fire (recipe target for hue_gradient).
    void compute(const ImageBuffer& im) {
        ...
        auto chroma2 = [](const Lab& p, const Lab& q) {
        ...
        for (int y = 0; y < im.height; ++y) {
            for (int x = 0; x < im.width; ++x) {
                ...
                const float dx = std::sqrt(std::max(0.0f, chroma2(r, l))) * 0.5f;
                const float dy = std::sqrt(std::max(0.0f, chroma2(d, u))) * 0.5f;
                gx.at(x, y) = dx;
                gy.at(x, y) = dy;
