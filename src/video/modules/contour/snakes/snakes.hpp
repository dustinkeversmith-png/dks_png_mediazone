...
public:
    ...
    Polyline evolve(const ImageBuffer& image, const Rect& bbox) const {
        ...
        return evolve_from(image, bbox, std::move(v));
    }

    Polyline evolve_from(const ImageBuffer& image, const Rect& bbox, std::vector<Vec2> v) const {
        if (static_cast<int>(v.size()) != n_points && v.size() >= 3) {
            // Resample caller polyline to n_points.
            float perim = 0;
            ...
            std::vector<Vec2> nv(static_cast<size_t>(n_points));
            for (int i = 0; i < n_points; ++i) {
                ...
                nv[static_cast<size_t>(i)] = a + (b - a) * ((t - acc[e - 1]) / seg);
            }
            v.swap(nv);
        }
        ...
        for (int it = 0; it < iterations; ++it) {
            ...
            for (int i = 0; i < static_cast<int>(v.size()); ++i) {
                ...
                const Vec2 outward = normalize({tang.y, -tang.x});
