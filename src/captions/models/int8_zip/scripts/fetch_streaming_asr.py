...
REPO_ROOT = Path(__file__).resolve().parents[5]     # repository root
...
def main():
    ...
    destination = ROOT / "models/compact"
    ...
    if args.runtime:
        # kaldi-native-fbank is ASR-only and stays with this model;
        # ONNX Runtime is shared with ../../../audio_synth, so it is
        # unpacked into the repository-wide dependencies directory.
        deps = ROOT / "third_party"
        shared = REPO_ROOT / "dependencies"
        shared.mkdir(parents=True, exist_ok=True)
        ort = shared / "downloads" / "ort.zip"
